...
___brainy_prompt_user_info() {
	...
	box="[|]"
	info="${color}\u ${bold_white}at ${color_m}\H"
	if [ -n "${SSH_CLIENT}" ]; then
		printf "%s|%s|%s" "${info}" "${bold_white}" "${box}"
	else
		printf "%s" "${info}"
...
THEME_SHOW_TODO=${THEME_SHOW_TODO:-"true"}
...
___BRAINY_TOP_LEFT=${___BRAINY_TOP_LEFT:-"dir user_info scm"}
___BRAINY_TOP_RIGHT=${___BRAINY_TOP_RIGHT:-"todo clock"}
...
__brainy_ps1() {
    bold=$(tput bold)
	printf "%s%s%s%s" "${bold}" "$(____brainy_top)" "$(____brainy_bottom)" "${normal}"
}

__brainy_ps2() {
	color=$bold_white
    bold=$(tput bold)
	printf "%s%s%s%s" "${bold}" "${color}" "${__BRAINY_PROMPT_CHAR_PS2}  " "${normal}"
}
